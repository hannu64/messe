...
function PrivateChat() {
  ...
  // Polling with server timestamp preservation
  useEffect(() => {
      ...
      try {
        ...
        setMessages(prevMessages => {
          ...
          const newOnes = incoming.map(rm => ({
            ...
            serverTimestamp: rm.timestamp || null // preserve backend ISO string
          }));
  ...
  const sendMessage = async () => {
    ...
    const msg = { 
      ...
      sender: 'me', 
      timestamp: Date.now(),
      serverTimestamp: null  // local send → no server ts yet
    };
        ...
        {decryptedMessages.map((msg, idx) => (
          ...
          >
            ...
            <div style={{
              ...
              {new Date(msg.serverTimestamp || msg.timestamp || Date.now()).toLocaleString([], {
                hour: '2-digit',
                minute: '2-digit',
                hour12: false
              })}
